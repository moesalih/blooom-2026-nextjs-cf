...
function parseItem(itemHtml: string): NewsItem | null {
	...
	return {
		...
		linkUrl: resolveUrl(headlineMatch[1], TECHMEME_ORIGIN),
		imageUrl: imageMatch ? resolveUrl(imageMatch[1], TECHMEME_ORIGIN) : null,
		source: extractSource(itemHtml),
	};
}

export function parseTechmemeFeed(html: string): NewsItem[] {
...
export async function fetchTechmemeNews(): Promise<NewsItem[]> {
