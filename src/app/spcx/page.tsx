...
export default function SpcxPage() {
	const { data, isPending, error } = useQuery({
		queryKey: ["financials", config.csvUrl],
		queryFn: async () => {
	...
	return (
		<div className="min-h-screen bg-background text-foreground">
			<main className="mx-auto w-full max-w-4xl px-4 py-8 sm:px-6">
				<h1 className="mb-8 text-xl font-semibold tracking-tight uppercase">
					{config.symbol}
				</h1>
				{isPending && (
				...
				{error && (
					<p className="text-destructive">Could not load financials.</p>
				)}
				{data && (
